        ...
        @foreach($notices as $n)
            <div class=" row rounded-2 shadow-sm mb-3">
                ...
                <div class="avatar-space col-2 ">
                    <div class="row">
                        <img href="{{'user-card/'}}{{$n->noticeUserId}}" src="{{ route('user.avatar',['filename'=>$n->imgUser]) }}"
                ...
                <div class="content-space col-6 ">
                    <div class="row my-2">
                        <a href="{{'post-card/'}}{{$n->id}}"><h1 class="tile-card-panel "> {{substr($n->title,0,25)}}</h1></a>
                        <p class="subtile-card-panel"> {{substr($n->description,0,100)}} ...</p>

                        <small class="align-text-bottom" style="bottom: 0px;position: absolute; font-size: 10px"> {{$n->created_at}}-{{ date('Y-m-d H:i:s') }}</small>
